"""Reroute suggestion agent pipeline: environment, staging, navigation, supervisor (LangGraph + optional Groq)."""
...
def _apply_prefer_final_if_no_farther_than_staging(
    *,
    ...
    """Clear warehouse staging when the leg to the original destination is no longer than the staging leg."""
...
async def suggest_reroute(
    *,
    ...
    async def staging_warehouse_agent(state: AgentState) -> dict[str, Any]:
    ...
    async def navigation_agent(state: AgentState) -> dict[str, Any]:
